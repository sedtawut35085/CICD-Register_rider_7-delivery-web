...
const ConfirmCodePage = ({userEmail, userPassword}) => {
    ...
    const [messageResendCode, setmessageResendCode] = useState(false)

    const resendConfirmationCode = async (e) => {
        try {
            await Auth.resendSignUp(userEmail);
            setmessageResendCode(true)
        } catch (err) {
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 h-screen w-full'>    
            ...
            <div className='bg-white flex flex-col mb-14 pb-5'>
                ...
                <form className='max-w-[400px] w-full mx-auto bg-white pl-8 pr-8 pt-16 justify-center mt-4 ' onSubmit={confirmCode}>
                    ...
                    {messageResendCode === false?
                        <>
                            <div className='pt-2'>
                                <h2 className='text-red-500 text-sm underline text-center' onClick={resendConfirmationCode}>คลิกเพื่อส่งรหัสอีกครั้ง</h2>
                            </div>
                        </>
                        :
                        null
                    }
                    {messageResendCode === true?
                        <>
                            <div className='pt-2'>
